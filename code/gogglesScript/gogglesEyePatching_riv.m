...
%Put in betterjjjjjjjj weber function formula for perceived contrast?
...
%         Screen('DrawTexture', wptr, tex2, Screen('Rect', tex2), CenterRect(Screen('Rect', tex2)*shrink_factor,Screen('Rect', wptr)),0,0);
        ...
        imgRect = CenterRect(Screen('Rect', tex(2))*shrink_factor,Screen('Rect', wptr));
        Screen('Close',tex);
        ...
        if keyIsDown&&(GetSecs-Tpress)>0.2
            ...
            elseif keyCode(KbName('LeftControl')) && keyCode(KbName('r'))
                stop(vid1);
                thisTrial = doRivalryTaskReal(stim,exp,controls, img, imgRect);
                 if tcount == 1
                     alltrials = thisTrial;
                 else
